...
if [ ! "$1" ]; then exit 1; fi
if [ ! -d ~/immudex ]; then exit 1; fi
if [ ! /sbin/debootstrap ]; then exit 1; fi

if [ "$1" = "--amd64" ] || [ "$1" = "--i386" ]; then
  if [ "$1" = "--amd64" ]; then arch="64"; else arch="32"; fi
  sudo rm -rf ~/immudex/${arch}/chroot;
  sudo /sbin/debootstrap --arch=$(echo $1 | sed 's/-//g') stable ~/immudex/${arch}/chroot http://deb.debian.org/debian
...
tidy;
EOF
  sudo cp base_chroot.sh ~/immudex/${arch}/chroot;
  sudo chroot ~/immudex/${arch}/chroot bash base_chroot.sh;
  sudo rm ~/immudex/${arch}/chroot/base_chroot.sh;
else
  exit 1;
fi 
