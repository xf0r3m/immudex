...
cd Python-3.11.0.tar.xz;

sudo ./configure --prefix=/usr/local --enable-optimizations
sudo make
sudo make altinstall

sudo sed -i '26s/1/0/' /etc/skel/.config/xfce4/xconf/xfce-perchannel-xml/xfce4-panel.xml;
sudo sed -i '64i\\t<property name="disable-struts" type="bool" value="false"/>' /etc/skel/.conf/xfce4/xconf/xfce-perchannel-xml/xfce4-panel.xml;
